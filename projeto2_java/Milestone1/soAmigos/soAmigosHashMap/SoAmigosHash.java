...
public class SoAmigosHash{
   
    HashMap<Integer,PerfilHashMap> hashNif;
    HashMap<String,PerfilHashMap> hashNome;
   
       public SoAmigosHash() {
        hashNif = new HashMap<Integer,PerfilHashMap>();
        hashNome = new HashMap<String,PerfilHashMap>();
    }
    
    public void inserirPerfil(String nome, long nif, String morada) {
        PerfilHashMap novo = new PerfilHashMap(new Utilizador(nome,nif,morada));
        hashNif.put((new Long(nif)).hashCode(),novo);
        hashNome.put(nome,novo);
    }
   
   public void removerPerfil(long nif) {
       PerfilHashMap p=hashNif.remove((new Long(nif)).hashCode());
       if(p!=null) hashNome.remove((p.getUtilizador().getNome()));
    }
    
    public void removerPerfil(String nome) {
       PerfilHashMap p=hashNome.remove(nome);
       if(p!=null) hashNif.remove((new Long(p.getUtilizador().getNif())).hashCode());
    }
    
   public void addAmigo(long nifUtiliz,long nifAmig) {
        PerfilHashMap utilizador= hashNif.get((new Long(nifUtiliz)).hashCode());
        PerfilHashMap amigo= hashNif.get((new Long(nifAmig)).hashCode());
   ...
   public void addAmigo(String nomeU,String nomeA) {
        PerfilHashMap utilizador= hashNome.get(nomeU);
        PerfilHashMap amigo= hashNome.get(nomeA);
        if(utilizador!=null && amigo!=null) {
    ...
    public void enviarMensagem(String msg, long nifDest, long nifRem) {
        PerfilHashMap dest= hashNif.get((new Long(nifDest)).hashCode());
        if (dest!=null) {
           if(hashNif.containsKey((new Long(nifRem)).hashCode())) dest.addMensagem(nifRem,msg);
        }
    }
    
    public void enviarMensagem(String msg,String ndest,String nrem) {
        PerfilHashMap dest= hashNome.get(ndest);
        PerfilHashMap rem= hashNome.get(nrem);
    ...
    public void apagarMensagem(String msg,long nifDest,long nifRem) {// ainda não acabada
        PerfilHashMap utilizador= hashNif.get((new Long(nifRem)).hashCode());
        PerfilHashMap pDest= hashNif.get((new Long(nifDest)).hashCode());
        pDest.removeMsg(msg);
    }
    
    public void verAmigos(long nif){
        PerfilHashMap p = hashNif.get((new Long(nif)).hashCode()).clone();
        PerfilHashMap a ;        
    }
    ...
    public String toString(){
        ...
        for(PerfilHashMap p : hashNif.values()){
            s.append(p.toString());
        }
    ...
    return s.toString();
     ...
     public void tempoToString(){
         ...
         while (i < 5 )  {
            GregorianCalendar agora = new GregorianCalendar(); 
            System.out.println(hashNif.toString());
            GregorianCalendar fim = new GregorianCalendar();
            tempo+=(fim.getTimeInMillis() - agora.getTimeInMillis());
        i++;
        } 
    ...
    public void tempoPesquisaNome(){
      ...
      PerfilHashMap p=hashNome.get(nome);
      while (i < 10000 )  {
            startTime = System.nanoTime();
            p=hashNome.get(nome);
            endTime = System.nanoTime();
